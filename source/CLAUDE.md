...
**Build/publish scripts live in `source/__assets/<platform>/`** (`win/` = PowerShell `.ps1`, `linux/` = bash `.sh`), each taking the target arch (`x64`, plus `arm64` for the Windows scripts) and self-locating the `source/` dir; the VS Code tasks in `.vscode/tasks.json` are thin wrappers that invoke them. A future macOS head would add `__assets/mac/`.
