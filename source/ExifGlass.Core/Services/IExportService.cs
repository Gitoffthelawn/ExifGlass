...
{
    /// <summary>Writes <paramref name="rows"/> to <paramref name="destination"/> as UTF-8 in the given format.</summary>
    Task ExportAsync(
        ExportFileType type,
        Stream destination,
    ...
    /// <summary>Grouped, tab-separated plain text with <c>[Group]</c> headings.</summary>
    string BuildText(IReadOnlyList<ExifTagItem> rows);

    /// <summary>RFC-4180 CSV: header + one quoted row per tag.</summary>
    string BuildCsv(IReadOnlyList<ExifTagItem> rows);

    /// <summary>Indented JSON array via the source-generated serializer.</summary>
